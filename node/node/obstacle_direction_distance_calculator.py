...
class ObstacleDirectionDistanceCalculator(Node):
    def __init__(self):
        super().__init__('obstacle_direction_distance_calculator')
        ...
        self.robot_orientation = 0.0  # Robot's orientation in radians
    ...
    def pose_callback(self, msg):
        # Update robot's position and orientation from the pose message
        self.robot_position = np.array([msg.pose.pose.position.x, msg.pose.pose.position.y])
        # Assuming the orientation is in quaternion, convert to yaw
    ...
    def costmap_callback(self, msg):
        ...
        # Reset distances
        self.distances = {key: float('inf') for key in directions.keys()}
 ...
 ```python
    def quaternion_to_yaw(self, orientation):
        ...
        cosy_cosp = orientation.w * orientation.w - orientation.x * orientation.x - orientation.y * orientation.y + orientation.z * orientation.z
        return math.atan2(siny_cosp, cosy_cosp)

def main(args=None):
    rclpy.init(args=args)
    obstacle_direction_distance_calculator = ObstacleDirectionDistanceCalculator()
    rclpy.spin(obstacle_direction_distance_calculator)
    obstacle_direction_distance_calculator.destroy_node()
    rclpy.shutdown()
