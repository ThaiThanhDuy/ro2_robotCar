...
class DijkstraPlanner(Node):
 ...
 return []
    ...
    def follow_path(self, path):
        ...
        cmd_vel.linear.x = min(distance, 0.5)  # Cap the speed to 0.5 m/s

        # Set angular velocity (turn towards the target)
        if distance > 0:
            cmd_vel.angular.z = np.arctan2(direction_y, direction_x)  # Angle to the target
        else:
            cmd_vel.angular.z = 0.0  # No rotation if already at the target

        # Calculate yaw from angular velocity (for logging purposes)
        yaw = np.arctan2(direction_y, direction_x)  # Yaw angle towards the target

        # Log the cmd_vel values
        self.get_logger().info(f'cmd_vel - Linear X: {cmd_vel.linear.x}, Angular Z: {cmd_vel.angular.z}, Yaw: {yaw}')
