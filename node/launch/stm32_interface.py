...
from std_msgs.msg import Float64MultiArray  # For joint commands
import serial
import tkinter as tk
from threading import Thread
from sensor_msgs.control import JointStateHardwareInterface
...
class STM32Interface(Node):
    def __init__(self):
        ...
        self.hw_interface = JointStateHardwareInterface()
        self.hw_interface.register_joint('front_left_wheel', 'position', 'velocity', 'effort')
        self.hw_interface.register_joint('front_right_wheel', 'position', 'velocity', 'effort')
        self.hw_interface.register_joint('back_left_wheel', 'position', 'velocity', 'effort')
        self.hw_interface.register_joint('back_right_wheel', 'position', 'velocity', 'effort')
    ...
    def joint_command_callback(self, msg):
        # Receive joint commands from the control manager
        FL, FR, BL, BR = msg.data
        self.send_command(FL, FR, BL, BR)

    def send_command(self, FL, FR, BL, BR):
        # Send the command to STM32
        command = f"cmd: FL:{FL} FR:{FR} BL:{BL} BR:{BR}\n"
    ...
    def start_gui(self):
        ...
        send_button = tk.Button(self.root, text="Send Command", command=self.send_command_from_gui)
    ...
    def send_command_from_gui(self):
        try:
            ...
            v_FL = v_x + v_y + v_theta  # Front Left
            v_FR = v_x - v_y - v_theta  # Front Right
            v_BL = v_x - v_y + v_theta  # Back Left
            v_BR = v_x + v_y - v_theta  # Back Right
            ...
            # Send the command to STM32
            self.send_command(v_FL, v_FR, v_BL, v_BR)
        except ValueError:
